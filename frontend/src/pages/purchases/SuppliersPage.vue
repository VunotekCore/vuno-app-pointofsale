    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 mb-4">
      ...
      <div class="lg:hidden p-3 border-b border-slate-200 dark:border-slate-800">
        ...
        >
          ...
          <span v-if="searchQuery || statusFilter" class="px-1.5 py-0.5 bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 text-xs rounded-full">
            {{ [searchQuery && 'Búsqueda', statusFilter && 'Estado'].filter(Boolean).length }}
          </span>
        </button>
      </div>
