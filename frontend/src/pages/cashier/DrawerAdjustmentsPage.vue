...
<template>
  <div class="p-4 md:p-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-xl md:text-2xl font-bold text-slate-900 dark:text-white">Ajustes de Caja</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">Gestiona faltantes y sobrantes de dinero</p>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 mb-4">
      ...
      <div class="lg:hidden p-3 border-b border-slate-200 dark:border-slate-800">
        ...
        >
          ...
          <span v-if="searchQuery || selectedStatus" class="px-1.5 py-0.5 bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 text-xs rounded-full">
            {{ [searchQuery && 'Búsqueda', selectedStatus && 'Estado'].filter(Boolean).length }}
          </span>
        </button>
      </div>
