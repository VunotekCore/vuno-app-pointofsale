...
import { Loader2, User, Check } from 'lucide-vue-next'
...
const fetchUserPermissions = async (userId) => {
  ...
  try {
    ...
    for (const p of perms) {
      newUserPermissions[p.permission_id || p.id] = true
    }
    userPermissions.value = newUserPermissions
  } catch (error) {
...
const hasUserPermission = (permId) => userPermissions.value[permId] === true

const toggleUserPermission = (permId) => {
  if (selectedUserId.value) {
    userPermissions.value[permId] = !userPermissions.value[permId]
  }
}

const saveUserPermissions = async () => {
  if (!selectedUserId.value) { notification.warning('Selecciona un usuario primero'); return }
  try {
    saving.value = true
    const permIds = Object.entries(userPermissions.value).filter(([_, v]) => v).map(([id]) => id)
    await api.put(`/permissions/user/${selectedUserId.value}`, { permissionIds: permIds })
    notification.success('Permisos del usuario guardados')
...
watch(searchUsers, () => { tablePage.value = 1; viewPage.value = 1 })
...
<template>
  <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
    <div class="p-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-slate-900 dark:text-white">Asignar Permisos Directos a Usuario</span>
        </div>
        <div class="flex items-center gap-2">
          <input v-model="searchUsers" placeholder="Buscar tabla..." class="w-48 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500" />
          <select v-model="selectedUserId" @change="fetchUserPermissions(selectedUserId)" class="px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-900 dark:text-slate-100">
            <option value="" disabled>Seleccionar usuario...</option>
            <option v-for="user in users" :key="user.id" :value="user.id">{{ user.username }}</option>
          </select>
          <button @click="saveUserPermissions" :disabled="saving || !selectedUserId" class="btn-primary flex items-center gap-1 px-3 py-2">
    ...
    <div v-if="!selectedUserId" class="p-8 text-center text-slate-500 dark:text-slate-400">Selecciona un usuario para asignar permisos directos</div>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
              <tr v-for="[tableName, perms] in paginatedTableGroups" :key="tableName" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <td class="px-4 py-3 text-center">
                  <button v-if="perms.read" @click="toggleUserPermission(perms.read.id)" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="hasUserPermission(perms.read.id) ? 'bg-green-500' : 'bg-red-500'">
                    <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="hasUserPermission(perms.read.id) ? 'translate-x-6' : 'translate-x-1'" />
                  </button>
                  <span v-else class="text-slate-300">-</span>
                </td>
                <td class="px-4 py-3 text-center">
                  <button v-if="perms.write" @click="toggleUserPermission(perms.write.id)" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="hasUserPermission(perms.write.id) ? 'bg-green-500' : 'bg-red-500'">
                    <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="hasUserPermission(perms.write.id) ? 'translate-x-6' : 'translate-x-1'" />
                  </button>
                  <span v-else class="text-slate-300">-</span>
                </td>
                <td class="px-4 py-3 text-center">
                  <button v-if="perms.delete" @click="toggleUserPermission(perms.delete.id)" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="hasUserPermission(perms.delete.id) ? 'bg-green-500' : 'bg-red-500'">
                    <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="hasUserPermission(perms.delete.id) ? 'translate-x-6' : 'translate-x-1'" />
                  </button>
                  <span v-else class="text-slate-300">-</span>
                </td>
              </tr>
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
              <tr v-for="[resourceName, perms] in paginatedViewGroups" :key="resourceName" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <td class="px-4 py-3 text-center">
                  <button v-if="perms.access" @click="toggleUserPermission(perms.access.id)" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" :class="hasUserPermission(perms.access.id) ? 'bg-green-500' : 'bg-red-500'">
                    <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="hasUserPermission(perms.access.id) ? 'translate-x-6' : 'translate-x-1'" />
                  </button>
                </td>
              </tr>
            </tbody>
